...
namespace ApiPlan.Controllers
{
    ...
    public class TPlTruckLoadListController : EntityControllerBase<TPlTruckLoadList, TPlTruckLoadListReadModel, TPlTruckLoadListCreateModel, TPlTruckLoadListUpdateModel>
    {
        ...
        {
            var repo = DataContext.Set<TTrTruckBayQueue>();
            var repoRc = DataContext.Set<TRcTruckBayQueue>();
            var queueInfo = repo.FirstOrDefault(t => t.BayCode == bayCode && t.TruckNo == vehicleNo && t.MainLoadingNo == mainTruckLoadingNo);
            if (queueInfo != null)
            {
                queueInfo.DispatchStepState = 4;
                queueInfo.OutBayTime = DateTime.Now;
                queueInfo.UpdateTime = DateTime.Now;

                TTrTruckBayQueue pushQueue = null;
                int queueCount = 0;
                bool isDel = true;
                var otherBayList = new List<TTrTruckBayQueue>();
                if (queueInfo.IsMoreBay == 1)  //多库房判断是否全部装车完成
                {
                    otherBayList = repo.Where(t => t.BayCode != bayCode && t.MainLoadingNo == queueInfo.MainLoadingNo && t.DispatchStepState <= 3).OrderBy(t => t.BayCode).ToList();
                ...
                if (isDel)
                {
                    ...
                    rcQueueInfo.Id = IdWorkerGenerator.Instance.NextId().ToString();
                    //rcQueueInfo.Remark = $"已离库";
                    repoRc.Add(rcQueueInfo);
                    repo.Remove(queueInfo);
                    queueCount = -1;
                }
                else  //不能删除则一定是多库房
                {
                    queueInfo.Remark = $"本库已离库，请前往下一库房装车";
                    #region 现在方式从南到北依次排队，不考虑车辆多少

                    var needQueueBay = otherBayList.Select(t => t.BayCode).FirstOrDefault();
                    var curQueueBayList = otherBayList.Where(t => t.IsMoreBay == 1 && t.BayCode == needQueueBay && t.DispatchStepState == 1 && t.CreateTime > DateTime.Now.AddHours(-8)).ToList();
                    var needQueueTruck = repo.FirstOrDefault(t => t.MainLoadingNo == mainTruckLoadingNo && t.BayCode == needQueueBay);
                    var sortNo = curQueueBayList.Max(t => t.SortNo);
                    if (sortNo == 0 || sortNo == null)
                    {
                        needQueueTruck.SortNo = 1000;
                    }
                    else
                    {
                        needQueueTruck.SortNo = sortNo + 10;
                    }
                    needQueueTruck.DispatchStepState = 1;
                    needQueueTruck.UpdateTime = DateTime.Now;
                    needQueueTruck.Remark += $";已从{queueInfo.BayCode}离库，前往{needQueueBay}装车";
                    #endregion
                }
                await DataContext.SaveChangesAsync(cancellationToken);
            }
