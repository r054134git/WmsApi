...
{
    /// <summary>
    /// Validator class for <see cref="TPlMaterialUpdateModel"/> .
    /// </summary>
    public partial class TPlMaterialUpdateModelValidator
        : AbstractValidator<TPlMaterialUpdateModel>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="TPlMaterialUpdateModelValidator"/> class.
        /// </summary>
        public TPlMaterialUpdateModelValidator()
        {
            ...
            RuleFor(p => p.ProductPackTime).MaximumLength(20);
            RuleFor(p => p.LockReason).MaximumLength(200);
            RuleFor(p => p.Pono).MaximumLength(10);
            RuleFor(p => p.HeatNo).MaximumLength(12);
            RuleFor(p => p.SteelGrade).MaximumLength(30);
            RuleFor(p => p.TrimFlag).MaximumLength(5);
            ...
            RuleFor(p => p.PrePickLocation).MaximumLength(30);
            RuleFor(p => p.DefectRecorder).MaximumLength(20);
            RuleFor(p => p.DefectDesc).MaximumLength(255);
            RuleFor(p => p.BayName).MaximumLength(50);
            RuleFor(p => p.MaterialLocationOwner).NotEmpty();
            RuleFor(p => p.MaterialLocationOwner).MaximumLength(2);
            RuleFor(p => p.MaterialBussinessOwner).MaximumLength(3);
