...
namespace WmsApi.Controllers
{
    ...
    public class DashboardController : QueryControllerBase<TRcLog, TRcLogReadModel, TRcLogCreateModel, TRcLogUpdateModel>
    {
        ...
        /// <returns></returns>
        [HttpGet("")]
        public async Task<ActionResult<PageResult<BLModel>>> GetBIListNew([FromQuery] PageParam pageParam, [FromQuery] string[] time, CancellationToken cancellationToken)
        {
            var rcCraneTasks = DataContext.Set<TRcCraneTask>();
            var rcMaterialEvents = DataContext.Set<TRcMaterialEvent>();
            var whBays = DataContext.Set<TWhBay>();
            ...
            // 过滤启用的 bay
            var query = whBays.Where(t => t.IsEnable == 1);
            var baycount = await query.CountAsync(cancellationToken);
            var baylist = await query.Where(b => b.Code != "18" && b.Code != "19" && b.Code != "20").ToListAsync(cancellationToken);
            ...
            // 定义通用的过滤条件
            var materialEventFilter = rcMaterialEvents.Where(t => t.Dt > startime && t.Dt <= endtime && t.Result == "成功");
            var craneTaskFilter = rcCraneTasks.Where(t => t.StartTime > startime && t.EndTime <= endtime);
            ...
            {
                // 定义 bay 相关的过滤条件
                var bayCode = bay.Code;
                var bayMaterialEventFilter = materialEventFilter.Where(t =>
                    EF.Functions.Like(t.FromLocationName, $"{bayCode}%") || EF.Functions.Like(t.ToLocationName, $"{bayCode}%"));

                // 自动入库
                var inputZ = await bayMaterialEventFilter.Where(t => t.OperationType == "自动" && t.ContentType == "入库").CountAsync(cancellationToken);

                // 手动入库
                var inputR = await bayMaterialEventFilter.Where(t => t.OperationType == "人工" && t.ContentType == "入库"
                    && !string.IsNullOrEmpty(t.CraneName)
                    && (EF.Functions.Like(t.FromLocationName, $"{bayCode}0M%") || EF.Functions.Like(t.FromLocationName, $"{bayCode}0N%"))).CountAsync(cancellationToken);

                // 自动出库
                var outZ = await bayMaterialEventFilter.Where(t => t.OperationType == "自动" && t.ContentType == "出库"
                    && EF.Functions.Like(t.FromLocationName, $"{bayCode}0%")
                    && EF.Functions.Like(t.ToLocationName, $"{bayCode}P%")).CountAsync(cancellationToken);

                // 人工出库
                var outR = await bayMaterialEventFilter.Where(t => t.OperationType == "人工" && t.ContentType == "出库"
                    && !string.IsNullOrEmpty(t.CraneName)
                    && EF.Functions.Like(t.FromLocationName, $"{bayCode}0%")
                    && EF.Functions.Like(t.ToLocationName, $"{bayCode}P%")).CountAsync(cancellationToken);

                // 人工出库(系统判定)
                var outRsys = await bayMaterialEventFilter.Where(t => t.OperationType == "人工" && t.ContentType == "出库"
                    && !string.IsNullOrEmpty(t.CraneName)
                    && EF.Functions.Like(t.FromLocationName, $"{bayCode}0%")
                    && t.Description.Contains("装车或是地面")).CountAsync(cancellationToken);

                // 自动倒垛
                var ddZ = await bayMaterialEventFilter.Where(t => t.OperationType == "自动"
                    ...
                    && !t.ToLocationName.Contains($"{bayCode}P")).CountAsync(cancellationToken);

                // 人工倒垛
                var ddR = await bayMaterialEventFilter.Where(t => t.OperationType == "人工"
                    && EF.Functions.Like(t.FromLocationName, $"{bayCode}%")
                    && EF.Functions.Like(t.ToLocationName, $"{bayCode}%")
                    && !t.FromLocationName.Contains($"{bayCode}0M")
                    && !t.FromLocationName.Contains($"{bayCode}0N")
                ...
                // 过跨倒运
                var ddGK = await craneTaskFilter.Where(t => t.PlanTypeName == "过跨倒运"
                    && EF.Functions.Like(t.BayName, $"{bayCode}%")).CountAsync(cancellationToken);

                // 库间倒运
                var ddKJ = await craneTaskFilter.Where(t => t.PlanTypeName == "库间倒运"
                    && EF.Functions.Like(t.BayName, $"{bayCode}%")).CountAsync(cancellationToken);
